...
const char *__fastcall FactionRec::GetFilename() {
...
bool FactionRec::Read(SFile *f, const char *stringBuffer) {
  ...
  result = SFile::Read(f, &m_reputationRaceMask[0], sizeof(m_reputationRaceMask), 0, 0, 0) && result;
  result = SFile::Read(f, &m_reputationClassMask[0], sizeof(m_reputationClassMask), 0, 0, 0) && result;
  result = SFile::Read(f, &m_reputationBase[0], sizeof(m_reputationBase), 0, 0, 0) && result;
  ...
  if (!result) {
    ConsoleWrite("Error reading FactionRec", DEFAULT_COLOR);
    return false;
  }
