...
void CParticleEmitter2::Sync() {
  unsigned int arraySize = static_cast<unsigned int>(m_particleLifeSpan * m_particleEmissionRate * 1.15f);
  ...
  for (unsigned int index = 0; index < 4; ++index) {
    if (m_childEmitter[index]) {
      unsigned int childSize =
          arraySize * static_cast<unsigned int>(m_childEmitter[index]->m_particleLifeSpan * m_childEmitter[index]->m_particleEmissionRate * 1.15f);
...
void CParticleEmitter2::SyncReserve(unsigned int arraySize, unsigned int oldSize, unsigned int oldReserve) {
...
void CParticleEmitter2::SyncAllocation(unsigned int arraySize) {
  if (m_particleType == PT_QUAD) {
    unsigned int oldSize = m_particles.Count();
    ...
    unsigned int oldSize = m_modelParticles.Count();
    if (oldSize >= arraySize) {
      return;
    }
...
int CParticleEmitter2::IRenderParticle(CParticle2 &p, CGxVertexPNCT0 *vtx) {
  ...
  if (m_twinkleOnOff < 1.0f || m_twinkleScaleRange != 0.0f) {
    randomIndex = ((reinterpret_cast<unsigned long>(&p) >> 5) + NTempest::CMath::ftol_0_256_(m_twinkleFPS * p.m_age)) & 0x7F;
  }
  ...
  if (m_particleHasHead) {
    unsigned int cell = static_cast<unsigned int>(headCell);
    ...
    if (m_particleAngularVelocity == 0.0f) {
      ...
      if (m_tumbler && (reinterpret_cast<unsigned long>(&p) & 0x20)) {
        theta = -theta;
      }
  ...
  unsigned int cell = static_cast<unsigned int>(tailCell);
  float        texU = (cell % m_textureColumns) * m_ooTextureWidth;
  float        texV = (cell / m_textureColumns) * m_ooTextureHeight;
...
void CParticleEmitter2::IRenderIndices(const CGxBufCommand &cmd, CGxBuf *buf) {
  ...
  if (cmd.index.op == GxBufOp_Fill) {
    ...
    indices = reinterpret_cast<unsigned short *>(*cmd.index.mem[GxVM_Indices]);
  } else if (cmd.index.op == GxBufOp_Assign) {
    ASSERT(cmd.index.mem[GxVM_Indices]);
    indices = reinterpret_cast<unsigned short *>(GxAllocIndexMem(buf->IndexCount() * sizeof(unsigned short)));
  ...
  unsigned short vertexBase = 0;
  for (unsigned int particle = 0; particle < s_renderedParticles; ++particle) {
    ...
    vertexBase = static_cast<unsigned short>(vertexBase + m_verticesPerParticle);
  }
}
...
void CParticleEmitter2::SetLifeSpan(float lifeSpan) {
  ...
  for (unsigned int i = 0; i < m_particleKeys.Count(); ++i) {
...
void CParticleEmitter2::SetReplaceableId(unsigned int id) {
  m_replaceableId = id;
}

void CParticleEmitter2::SetKey(unsigned int keyNdx, const CParticleKey &key) {
  ASSERT(keyNdx < m_particleKeys.Count());
  m_particleKeys[keyNdx] = key;
}

void CParticleEmitter2::SetTextureDimensions(unsigned int rows, unsigned int columns) {
  ...
  m_textureLog = static_cast<unsigned int>(-1);
  do {
    columns >>= 1;
    ++m_textureLog;
  } while (columns);
...
void CParticleEmitter2::SetParticleStyle(int hasHead, int hasTail, float tailLength, bool tailGrows) {
  ...
  unsigned int styleCount = m_particleHasHead + m_particleHasTail;
...
unsigned int CParticleEmitter2::ReplaceableId() {
  return m_replaceableId;
}

const CParticleKey &CParticleEmitter2::Key(unsigned int keyNdx) {
  return m_particleKeys[keyNdx];
}

void CParticleEmitter2::TextureDimensions(unsigned int &rows, unsigned int &columns) {
  rows = m_textureRows;
...
void CParticleEmitter2::InternalUpdate(float elapsedTime, int suppressNewParticles) {
  ...
  if (elapsedTime <= s_maxTimeStep) {
    ...
    unsigned char steps = NTempest::CMath::ftol_0_256_(numSteps);
