...
namespace NTempest {
  ...
  class C2Vector {
   public:
    ...
    C2Vector asC2Vector() const { return *this; }
    const C2Vector *asFloatPtr() const { return this; }
    ...
    C2Vector &operator+=(float a) { x += a; y += a; return *this; }
    C2Vector &operator+=(const C2Vector &a) { x += a.x; y += a.y; return *this; }
    C2Vector &operator-=(float a) { x -= a; y -= a; return *this; }
    C2Vector &operator-=(const C2Vector &a) { x -= a.x; y -= a.y; return *this; }
    C2Vector &operator*=(float a) { x *= a; y *= a; return *this; }
    C2Vector &operator*=(const C2Vector &a) { x *= a.x; y *= a.y; return *this; }
    C2Vector &operator/=(float a) { float inverse = 1.0f / a; x *= inverse; y *= inverse; return *this; }
    C2Vector &operator/=(const C2Vector &a) { x /= a.x; y /= a.y; return *this; }
    C2Vector operator-() const { return C2Vector(-x, -y); }
    float &operator[](unsigned int sub) { ASSERT(sub < 2); return (&x)[sub]; }
    const float &operator[](unsigned int sub) const { ASSERT(sub < 2); return (&x)[sub]; }
    ...
    float SumC() const { return x + y; }
    bool IsUnit() const { return CMath::fabs_(SquaredMag() - 1.0f) < 0.0009765625f; }
    float AxisAngle() const { return CMath::atan2_(y, x); }
    float AxisAngle(float angle) const { return AngleToAxisAngle(AxisAngle() - angle); }
  ...
  inline C2iVector::C2iVector(const C2Vector &vector)
