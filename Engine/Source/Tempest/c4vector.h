...
namespace NTempest {
  ...
  class C4Vector {
   public:
    ...
    C4Vector(float xValue, float yValue, float zValue, float wValue)
    ...
    const C4Vector *asFloatPtr() const { return this; }
    ...
    C4Vector &operator+=(float a) { x += a; y += a; z += a; w += a; return *this; }
    C4Vector &operator+=(const C4Vector &a) { x += a.x; y += a.y; z += a.z; w += a.w; return *this; }
    C4Vector &operator-=(float a) { x -= a; y -= a; z -= a; w -= a; return *this; }
    C4Vector &operator-=(const C4Vector &a) { x -= a.x; y -= a.y; z -= a.z; w -= a.w; return *this; }
    C4Vector &operator*=(float a) { x *= a; y *= a; z *= a; w *= a; return *this; }
    C4Vector &operator*=(const C4Vector &a) { x *= a.x; y *= a.y; z *= a.z; w *= a.w; return *this; }
    C4Vector &operator/=(float a) { return *this *= 1.0f / a; }
    C4Vector &operator/=(const C4Vector &a) { x /= a.x; y /= a.y; z /= a.z; w /= a.w; return *this; }
    C4Vector operator-() const { return C4Vector(-x, -y, -z, -w); }
    float &operator[](unsigned int index) { ASSERT(index < 4); return (&x)[index]; }
    const float &operator[](unsigned int index) const { ASSERT(index < 4); return (&x)[index]; }

    float SquaredMag() const { return x * x + y * y + z * z + w * w; }
    float Mag() const { return CMath::sqrt_(SquaredMag()); }
    float SumC() const { return x + y + z + w; }
    bool IsUnit() const { return CMath::fabs_(SquaredMag() - 1.0f) < 0.0009765625f; }
    void Normalize() { *this *= 1.0f / Mag(); }
    void SafeNormalize() {
  ...
  inline C4iVector::C4iVector(const C4Vector &vector)
      : x(static_cast<long>(vector.x)), y(static_cast<long>(vector.y)),
        z(static_cast<long>(vector.z)), w(static_cast<long>(vector.w)) {
  }
