...
namespace NTempest {
  ...
  C3Vector __fastcall operator*(const C3Vector &l, const C34Matrix &r) {
    return C3Vector(
        l.x * r.a0 + l.y * r.b0 + l.z * r.c0 + r.d0, l.x * r.a1 + l.y * r.b1 + l.z * r.c1 + r.d1, l.x * r.a2 + l.y * r.b2 + l.z * r.c2 + r.d2
    );
  }
  ...
  C34Matrix __fastcall C34Matrix::Rotation(float angle, const C3Vector &axis, bool unit) {
    C3Vector axis_(axis);
