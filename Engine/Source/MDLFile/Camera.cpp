...
static void IWriteCamera(
    const MDLCAMERASECTION &section,
    TSGrowableArray<char> &buffer
) {
  WriteLine(buffer, "%s \"%s\" {\n", TokenText(0x114), static_cast<const char *>(section.name));
  WriteLine(
      buffer,
      "\t%s { %g, %g, %g },\n",
      TokenText(0x1A5),
  ...
  WriteLine(
      ...
      "\t\t%s { %g, %g, %g },\n",
      TokenText(0x1A5),
      section.target.pivot.x,
      section.target.pivot.y,
      section.target.pivot.z
  );
  if (section.target.transkeys.keys.Count()) {
    WriteLine(
        ...
        "\t\t%s %d {\n",
        TokenText(0x1C7),
        section.target.transkeys.keys.Count()
    );
    WriteTrackHeader("\t\t", section.target.transkeys, buffer);
    for (unsigned int i = 0; i < section.target.transkeys.keys.Count(); ++i) {
      const MDLKEYFRAME<NTempest::C3Vector> &key =
          section.target.transkeys.keys.Ptr()[i];
      WriteLine(buffer, "\t\t\t%d: ", key.time);
      WriteKeyData(buffer, &key.value.x, 3);
...
static unsigned int GetBinCameraSize(const MDLCAMERASECTION &section) {
  ...
  if (section.rollkeys.keys.Count()) {
    unsigned int dataSize =
        section.rollkeys.type > TRACK_LINEAR ? 12 : 4;
    size += 16 + section.rollkeys.keys.Count() * (4 + dataSize);
  }
  if (section.target.transkeys.keys.Count()) {
    unsigned int dataSize =
        section.target.transkeys.type > TRACK_LINEAR ? 36 : 12;
...
static void IWriteBinCamera(
  ...
  if (section.target.transkeys.keys.Count()) {
    ...
    unsigned int values =
        section.target.transkeys.type > TRACK_LINEAR ? 9 : 3;
    for (unsigned int i = 0; i < section.target.transkeys.keys.Count(); ++i) {
      const MDLKEYFRAME<NTempest::C3Vector> &key =
          section.target.transkeys.keys.Ptr()[i];
      buffer.AddInt(key.time);
