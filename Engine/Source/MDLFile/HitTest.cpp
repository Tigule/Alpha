...
namespace MDL {
  ...
  int ReadHitTest(Parser &parse, MDLDATA &data, CMDLStatus *status) {
    ...
    switch (section->type) {
      ...
      case SHAPE_CYLINDER:
        ...
        section->shape.cylinder.height = vertices.Ptr()[1].z - vertices.Ptr()[0].z;
  ...
  static void IWriteHitTestSection(const MDLDATA &data, const MDLHITTESTSHAPE &section, int needObjectIds, TSGrowableArray<char> &buffer) {
    ...
    switch (section.type) {
      ...
      case SHAPE_CYLINDER:
        ...
        WriteLine(buffer, "\t}\n");
        WriteLine(buffer, "\t%s %g,\n", TokenText(0x134), section.shape.cylinder.radius);
        break;
      case SHAPE_SPHERE:
        WriteLine(buffer, "\t%s,\n", TokenText(0x1B4));
        ...
        WriteLine(buffer, "\t}\n");
        WriteLine(buffer, "\t%s %g,\n", TokenText(0x134), section.shape.sphere.radius);
        break;
      case SHAPE_PLANE:
        WriteLine(buffer, "\t%s,\n", TokenText(0x1A2));
        WriteLine(buffer, "\t%s %g,\n", TokenText(0x164), section.shape.plane.length);
        WriteLine(buffer, "\t%s %g,\n", TokenText(0x1DA), section.shape.plane.width);
        break;
    }
  ...
  int ReadBinHitTests(CMsgBuffer &buffer, UINT length, MDLDATA &data, CMDLStatus *status) {
    ...
    while (totalRead < length) {
      ...
      if (totalRead > length) {
        status->FatalOverran("Hit test section overran read buffer.\n", -1);
        return 0;
      }
