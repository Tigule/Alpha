...
static void IReadLightKeyFrames(
    Parser &parse,
    unsigned int savedToken,
    const char *tokenText,
    MDLLIGHTSECTION *light,
    unsigned int
) {
  switch (savedToken) {
    case 0x11F:
...
static void IReadLightStaticData(
    Parser &parse,
    unsigned int savedToken,
    const char *tokenText,
    MDLLIGHTSECTION *light,
...
static int IReadLightProperties(
    Parser &parse,
    unsigned int savedToken,
    MDLLIGHTSECTION *light,
    unsigned int
...
static void IReadLight(
  ...
  while (token && token != '}') {
    ...
    if (!ReadObjectBody(parse, token, pivot, light, status)
        && !IReadLightProperties(parse, token, light, version)) {
      if (expectAnimation) {
...
static void IWriteLightProperties(
    const MDLLIGHTSECTION &section,
    TSGrowableArray<char> &buffer
) {
  unsigned int token = 0x188;
...
static void IWriteLightSection(
    const MDLDATA &data,
    const MDLLIGHTSECTION &section,
    int needObjIds,
    TSGrowableArray<char> &buffer
...
static unsigned int GetBinLightSize(const MDLLIGHTSECTION &section) {
  unsigned int size = GetBinGenObjectSize(section) + 48;
  const MDLKEYTRACK<float> *floatTracks[5] = {
      &section.attenstartkeys,
      &section.attenendkeys,
      &section.intensitykeys,
      &section.ambintensitykeys,
      &section.visibilityKeys
  };
  for (unsigned int i = 0; i < 5; ++i) {
    const MDLKEYTRACK<float> &track = *floatTracks[i];
    if (track.keys.Count()) {
      unsigned int dataSize = track.type > TRACK_LINEAR ? 12 : 4;
      size += 16 + track.keys.Count() * (4 + dataSize);
    }
  }
  const MDLKEYTRACK<C3Color> *colorTracks[2] = {
      &section.colorkeys,
      &section.ambcolorkeys
  };
  for (unsigned int j = 0; j < 2; ++j) {
    const MDLKEYTRACK<C3Color> &track = *colorTracks[j];
    if (track.keys.Count()) {
      unsigned int dataSize = track.type > TRACK_LINEAR ? 36 : 12;
      size += 16 + track.keys.Count() * (4 + dataSize);
    }
  }
  return size;
...
static void IWriteBinLightSection(
    const MDLLIGHTSECTION &section,
    CMsgBuffer &buffer,
    CMDLStatus *status
) {
...
static int ReadBinLight(
  ...
  while (localRead < sectionLength) {
    ...
    if (tag == 'SALK') {
      ...
      if (!ok) status->Add(STATUS_ERROR, "Error reading light attenstart keys.\n");
    } else if (tag == 'EALK') {
      ok = ReadBinFloatKeyFrames(light->attenendkeys, buffer, localRead);
      if (!ok) status->Add(STATUS_ERROR, "Error reading light attenend keys.\n");
    } else if (tag == 'CALK') {
      ok = ReadBinFloatKeyFrames(light->colorkeys, buffer, localRead);
      if (!ok) status->Add(STATUS_ERROR, "Error reading light color keys.\n");
    } else if (tag == 'IALK') {
      ok = ReadBinFloatKeyFrames(light->intensitykeys, buffer, localRead);
      if (!ok) status->Add(STATUS_ERROR, "Error reading light intensity keys.\n");
    } else if (tag == 'CBLK') {
