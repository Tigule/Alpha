...
int          ReadObjectBody(Parser &parse, UINT savedToken, NTempest::C3Vector *pivot, MDLGENOBJECT *object, CMDLStatus *status);
...
int          WriteBinGenObject(const MDLGENOBJECT &object, CMsgBuffer &buffer, CMDLStatus *status);
...
int          ReadBinQuatKeyFrames(MDLKEYTRACK<NTempest::C4Quaternion> &track, CMsgBuffer &buffer, UINT &totalRead);
