...
static void IReadRibbonEmitterKeyFrames(
    Parser &parse,
    unsigned int savedToken,
    const char *tokenText,
    MDLRIBBONEMITTER *emitter
) {
  switch (savedToken) {
    case 0x11C:
...
static void IReadRibbonEmitterStaticData(
    Parser &parse,
    unsigned int savedToken,
    const char *tokenText,
    MDLRIBBONEMITTER *emitter
...
static void IReadRibbonEmitter(
    Parser &parse,
    TSet &errors,
    MDLRIBBONEMITTER *emitter,
    CMDLStatus *status
...
static void IWriteRibbonEmitter(
    const MDLDATA &data,
    const MDLRIBBONEMITTER &emitter,
    int needObjIds,
...
static unsigned int GetRibbonFixedDataSize() {
  return 56;
}

static unsigned int GetBinRibbonEmitterSize(
    const MDLRIBBONEMITTER &section
) {
  unsigned int size =
      4 + GetBinGenObjectSize(section) + GetRibbonFixedDataSize();
  if (section.heightAbove.keys.Count()) {
    unsigned int dataSize =
        section.heightAbove.type > TRACK_LINEAR ? 12 : 4;
    size += 16 + section.heightAbove.keys.Count() * (4 + dataSize);
  }
  if (section.heightBelow.keys.Count()) {
    unsigned int dataSize =
        section.heightBelow.type > TRACK_LINEAR ? 12 : 4;
    size += 16 + section.heightBelow.keys.Count() * (4 + dataSize);
  }
  if (section.alphaKeys.keys.Count()) {
    unsigned int dataSize =
        section.alphaKeys.type > TRACK_LINEAR ? 12 : 4;
    size += 16 + section.alphaKeys.keys.Count() * (4 + dataSize);
  }
  if (section.colorKeys.keys.Count()) {
    unsigned int dataSize =
        section.colorKeys.type > TRACK_LINEAR ? 36 : 12;
  ...
  if (section.visibilityKeys.keys.Count()) {
    unsigned int dataSize =
        section.visibilityKeys.type > TRACK_LINEAR ? 12 : 4;
    size += 16 + section.visibilityKeys.keys.Count() * (4 + dataSize);
  }
  return size;
}

static void IWriteBinRibbonEmitter(
    const MDLRIBBONEMITTER &section,
    CMsgBuffer &buffer,
    CMDLStatus *status
) {
  ...
  if (section.colorKeys.keys.Count()) {
    ...
    for (unsigned int i = 0; i < section.colorKeys.keys.Count(); ++i) {
  ...
  if (section.textureSlot.keys.Count()) {
    ...
    for (unsigned int i = 0; i < section.textureSlot.keys.Count(); ++i) {
...
static int ReadBinRibbonEmitter(
    CMsgBuffer &buffer,
    MDLRIBBONEMITTER *ribbon,
    CMDLStatus *status,
  ...
  if (!ReadBinGenObject(*ribbon, buffer, status, localRead)) {
    status->Add(
        STATUS_ERROR,
        "Error reading gen object portion of RibbonEmitter.\n"
    );
    return 0;
  }
  buffer.GetUint();
  ...
  while (localRead < sectionLength) {
    ...
    if (tag == 'AHRK') {
      ...
      if (!ok) {
        status->Add(
            STATUS_ERROR,
            "Error reading height above portion of RibbonEmitter.\n"
        );
      }
    } else if (tag == 'BHRK') {
      ...
      if (!ok) {
        status->Add(
            STATUS_ERROR,
            "Error reading height below of RibbonEmitter.\n"
        );
      }
    } else if (tag == 'LARK') {
      ...
      if (!ok) {
        status->Add(
            STATUS_ERROR,
            "Error reading alpha portion of RibbonEmitter.\n"
        );
      }
    } else if (tag == 'OCRK') {
      ...
      if (!ok) {
        status->Add(
            STATUS_ERROR,
            "Error reading color portion of RibbonEmitter.\n"
        );
      }
    } else if (tag == 'XTRK') {
      ...
      if (!ok) {
        status->Add(
            STATUS_ERROR,
            "Error reading texture slot keys portion of RibbonEmitter.\n"
        );
      }
    } else if (tag == 'SIVK') {
      ...
      if (!ok) {
        status->Add(
            STATUS_ERROR,
            "Error reading visibility keys portion of RibbonEmitter.\n"
        );
      }
    } else {
